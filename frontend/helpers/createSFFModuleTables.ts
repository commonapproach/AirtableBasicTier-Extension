...
export async function createSFFModuleTables(intl: IntlShape) {
	...
	for (const tableToCreate of tablesToCreate) {
		...
		createdTables.push(tableToCreate);
	}

	// Create all regular fields
	for (const tableToCreate of tablesToCreate) {
		const tableClass = new mapSFFModel[tableToCreate as SFFModelType]();
		...
		switch (fieldType) {
			case FieldType.SINGLE_SELECT:
				if (field.getOptionsAsync) {
					const asyncOptions = await field.getOptionsAsync();
					options = { choices: (asyncOptions || []).map((v) => ({ name: v.name })) };
				} else {
